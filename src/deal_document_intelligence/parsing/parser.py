"""Parser interface — stages 1-3 (ingest → parse/OCR → structure).
    ...
    Output: a CanonicalDocument (normalised text + blocks with page/char offsets).
...
the contract: whatever you plug in must return a `CanonicalDocument`. You can
self-check your adapter with `CanonicalDocument.verify()` /
